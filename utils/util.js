...
import { customerType } from "@/api/api.js";
// #ifdef APP-PLUS
...
export default {
   ...
   * 省市区地址分解
   */
  addressInfo: function (str) {
    let reg = /.+?(省|市|自治区|自治州|行政区|盟|旗|县|区)/g; // 省市区的正则
    const area = str.match(reg); // 分割省市区
    let citys = ["北京市", "天津市", "上海市", "重庆市"];
    let address_component = {
      city: "",
      district: "",
      province: area[0],
    };
    if (citys.indexOf(area[0]) == -1) {
      address_component.city = area[1];
      address_component.district = area[2];
    } else {
      address_component.city = area[0];
      address_component.district = area[1];
    }
  ...
  PosterCanvas: function (
    ...
    uni.getImageInfo({
      ...
      success: function (res) {
        ...
        ctx.fillText("￥" + ot_price, 30, 1152 + contentHh);
        ...
        underline(ctx, "￥" + ot_price, 30, 1146, 32, "#999", 2, 0);
        ctx.save();
        ctx.setTextAlign("left");
        ctx.setFontSize(54);
  ...
  getFileAccessHttpUrl(url) {
    ...
    if (url && url.length > 0) {
      if (!env.staticURL.endsWith("/") && !url.startsWith("/")) {
  ...
  // 图片处理-上传图片
  uploadImage(api, url) {
    // let config_url = env.baseURL;
    console.log("图片处理-上传图片", api, url);
    return new Promise((resolve, reject) => {
      uni.uploadFile({
        // url: config_url + api,
        url: HTTP_REQUEST_URL + "/api/" + uploadUrl,
  ...
  // 年月日
  timestampRange(timestamp1, timestamp2) {
    let dateText1 = this.timestamp(timestamp1);
    let dateText2 = this.timestamp(timestamp2);
    let year = dateText1.split(".")[0];
    dateText2 = dateText2.replace(year, "");
   ...
   *options(其它参数)
   */
  msg({ title = "", duration = 1500, mask = true, icon = "none", fn }) {
    //统一提示方便全局修改
    uni.showToast({
      title,
      duration,
  ...
  probability(events) {
    ...
    events.forEach((item) => {
      item.min = total;
      total += Math.abs(Number(item.probability));
      item.max = total - 1;
    });
    return {
      shuffle: () => {
